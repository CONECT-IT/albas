  ...
  <div class="fixed inset-0 flex items-center justify-center z-50">
    <div class="bg-white rounded-2xl p-8 shadow-xl w-[600px]" @click.stop>
      ...
      <div class="flex justify-between items-center mb-6">
        ...
        <button class="text-2xl font-bold relative -top-6" @click="$emit('close')">✕</button>
      </div>
      ...
      <div class="grid grid-cols-2 gap-6">
        ...
        <div>
          ...
          <input v-model="nombre" type="text" class="w-full border rounded-md p-2" />
        </div>

        <!-- Fecha -->
        <div>
          <label class="block text-sm font-semibold mb-1">Fecha</label>
          <input v-model="fecha" type="date" class="w-full border rounded-md p-2" />
        </div>

        <!-- Celular -->
        <div>
          <label class="block text-sm font-semibold mb-1">Número de Celular</label>
          <input v-model="celular" type="text" class="w-full border rounded-md p-2" />
        </div>

        <!-- Tipo -->
        <div>
          <label class="block text-sm font-semibold mb-1">Tipo</label>
          <select v-model="tipo" class="w-full border rounded-md p-2">
      ...
      <div class="flex justify-end space-x-4 mt-8">
        <button @click="$emit('close')" class="px-4 py-2 border rounded-md">Cancelar</button>

        <button @click="crearLead" class="px-4 py-2 bg-black text-white rounded-md">
          Crear Lead
        </button>
      </div>
    </div>
